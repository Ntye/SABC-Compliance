# Postman collections — Puppet Enterprise APIs

A comprehensive Postman collection (v2.1 schema) for probing the Puppet
Enterprise APIs that SABC-Compliance integrates with, with a matching
environment so hosts/credentials/tokens are configured once and reused
everywhere.
...
2. Top-right environment selector → choose **Puppet**.
