...
\$COMPOSE down --remove-orphans 2>/dev/null || true
# Force-remove ALL three of our fixed-name containers (incl. sabc-postgres) so a
# stale container left by an earlier run — even one created under a different
# compose project name — cannot block recreation with a "name already in use"
# error or keep holding a published port. Only our own named containers are
# touched. The postgres DATA lives in the postgres-data named volume, which
# "docker rm -f" never removes, so this is safe and non-destructive.
docker rm -f sabc-frontend sabc-backend sabc-postgres 2>/dev/null || true
